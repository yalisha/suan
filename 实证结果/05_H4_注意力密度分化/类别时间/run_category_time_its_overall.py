#!/usr/bin/env python3
"""
按周聚合各细分类别（不区分官媒/非官媒），计算当日热搜时长相关指标的
前后（事件日期 2024-11-12，对齐到当周周一）ITS 变化，输出与原格式一致的结果。
"""
...
AGG_CSV = OUT_DIR / "weekly_category_time_overall.csv"
RESULT_XLSX = OUT_DIR / "ITS_category_time_overall.xlsx"
RESULT_CSV = OUT_DIR / "ITS_category_time_overall.csv"
...
def load_labels() -> pd.DataFrame:
    ...
    labels = pd.read_csv(LABELS_PATH, usecols=["host", "host_category_raw"])
...
def aggregate_weekly(frame: pd.DataFrame) -> pd.DataFrame:
    grouped = (
        frame.groupby(["week_start", "host_category_raw"], as_index=False)
        .agg(
    ...
    grouped = grouped.rename(columns={"host_category_raw": "category"})
    ordered_cols = [
        "week_start",
        "category",
